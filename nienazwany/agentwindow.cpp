...
void AgentWindow::on_pushButtonSaveAgentPath_clicked()
{
    ...
    for(int i = 0; i <= _Scene->getPath().size()-1; ++i)
        Trasa.push_back(Segment( Coordinates(_Scene->getPath().at(i).x,_Scene->getPath().at(i).y), Coordinates(_Scene->getPath().at(i).x,_Scene->getPat().at(i).y)) );
    Trasa.push_back(Segment( Coordinates(_Scene->getPath().at(size()-1).x,_Scene->getPath().at(size()-1).y), Coordinates(_Scene->getEndPos().x,_Scene->getEndPos().y)) );

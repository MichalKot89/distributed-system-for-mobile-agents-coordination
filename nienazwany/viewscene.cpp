...
void ViewScene::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if(buttonMode == Stop)
    {
        ...
        if (EndCoord == true)
            removeItem(itemAt(end.x,end.y));
        addEllipse(event->scenePos().x()-5,event->scenePos().y()-5,10,10,QPen(Qt::red),QBrush(Qt::darkRed));
        end.x=event->scenePos().x(); end.y=event->scenePos().y();
        EndCoord = true;
    ...
    if(buttonMode == Path)
    {
        ...
        coord Temp(event->scenePos().x(),event->scenePos().y());
        path.push_back(Temp);
        PathCoord = true;
    }

    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(_actStart, SIGNAL(triggered()), ui->simulationWindow, SLOT(on_pushButtonStart_clicked()));

    _actStop = new QAction(tr("&Stop"), this);
    connect(_actStop, SIGNAL(triggered()), ui->simulationWindow, SLOT(on_pushButtonStop_clicked()));

    _actReset = new QAction(tr("&Reset"), this);
    connect(_actReset, SIGNAL(triggered()), ui->simulationWindow, SLOT(on_pushButtonReset_clicked()));

    _actSave = new QAction(tr("&Zapisz"), this);
    connect(_actSave, SIGNAL(triggered()), ui->simulationWindow, SLOT(on_pushButtonZapisz_clicked()));

    _actLoad = new QAction(tr("&Wczytaj"), this);
    connect(_actLoad, SIGNAL(triggered()), ui->simulationWindow, SLOT(on_pushButtonWczytaj_clicked()));
